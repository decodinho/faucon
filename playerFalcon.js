window.lancerStreaming = function(url) {
    // 1. Création de l'interface (Overlay) sans condition de vérification
    ...
    // 2. Initialisation directe de Video.js
    ...
    document.getElementById('close-player').onclick = function() {
        player.dispose(); // Détruit le player et libère la mémoire
        overlay.remove(); // Supprime l'interface noire
    };
};
